module.exports = function(iter){
	if(this.options.console_logging.show && iter % this.options.console_logging.step == 0){
		...
		if(this.options.console_logging.show_error) console.log(`Training error: ${this.error}`);
		if(this.options.console_logging.show_test_error) console.log(`Test error: ${this.test_error}`);
